...
7. Filing or appending creates a Subscription. Polling catches close, reopen, edits, and Issues filed outside Slack, and keeps the index current. Close notifies by DM and thread reply, with distinct wording for completed versus not planned. Reopen replies in the thread. Deletion and transfer leave no trace in the polling API, so an Issue GitHub no longer serves is dropped from the index two ways: on sight, when retrieval or an Append touches it, and in bulk by `swatter sync`, which asks about every indexed Issue its full fetch did not return.
